...
class UnAuthenticatedTestCase(APITestCase):
    ...
    nonpublic_urls = ['/nonpublic/menu/', '/nonpublic/menu/menu/', '/nonpublic/dish/', '/nonpublic/dish/dish_name/', '/nonpublic/dish/dish_name/1/']

    def test_public_access(self):
        response = self.client.get('/menu/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_nonpublic_access(self):
        for url in self.nonpublic_urls:
...
class AuthenticatedTestCase2(APITestCase):
    ...
    urls_200 = ['/nonpublic/menu/', '/nonpublic/dish/',]

    urls_404 = ['/nonpublic/menu/menu/', '/nonpublic/dish/dish_name/', '/nonpublic/dish/dish_name/1/']
    ...
    def test_nonpublic_menu_get(self):
        self.client.force_login(self.user)
        ...
        response = self.client.get('/nonpublic/menu/')
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get('/nonpublic/menu/name1/')
        self.assertEqual(response.data['name'], 'name1')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get('/nonpublic/menu/name2/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_nonpublic_menu_post(self):
        self.client.force_login(self.user)
        data = {'name': 'test name', 'description': 'description', 'user': self.user}
        response = self.client.post('/nonpublic/menu/', data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['name'], 'test name')
        self.assertEqual(response.data['description'], 'description')

    def test_nonpublic_menu_delete(self):
        self.client.force_login(self.user)
        Menu.objects.create(name='menu1', description='Test description', user=self.user)
        response = self.client.delete(f'/nonpublic/menu/menu1/')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Menu.objects.count(), 0)

    def test_nonpublic_menu_put(self):
        self.client.force_login(self.user)
        Menu.objects.create(name='menu1', description='Test description', user=self.user)
        response = self.client.put(f'/nonpublic/menu/menu1/',
                                   data={'name': 'menu1', 'description': 'Updated description'})
        self.assertEqual(response.data['description'], 'Updated description')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_nonpublic_dish_get(self):
        self.client.force_login(self.user)
        ...
        response = self.client.get('/nonpublic/dish/')
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get('/nonpublic/dish/menu1/')
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['name'], 'dish for menu1')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get('/nonpublic/dish/menup/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        response = self.client.get(f'/nonpublic/dish/menu1/{dish_for_1.id + 12}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        response = self.client.get(f'/nonpublic/dish/menu1/{dish_for_1.id}/')
        self.assertEqual(response.data['id'], dish_for_1.id)

    def test_nonpublic_dish_delete_put(self):
        self.client.force_login(self.user)
        menu1 = Menu.objects.create(name='menu1', description='Test description', user=self.user)
        dish_for_1 = Dish.objects.create(name='dish for menu1', description='Test description', price=10,
                                         prepare_time=datetime.timedelta(days=0, seconds=800), menu=menu1)
        response = self.client.put(f'/nonpublic/dish/menu1/{dish_for_1.id}/',
                                   data={'name': 'dish for menu1', 'description': 'Updated description', 'price': 15,
                                         'prepare_time': datetime.timedelta(days=0, seconds=800), 'menu': menu1})
        self.assertEqual(response.data['description'], 'Updated description')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.delete(f'/nonpublic/dish/menu1/{dish_for_1.id}/')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
